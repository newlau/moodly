...
2026-06-29 轻微噪音误打断热修记录：

Status: current。

- 线上反馈“轻微噪音一直中断角色说话”后，播放中自动打断从“灵敏插话优先”调整为“强确认人声优先”。轻微环境声、外放回声、摩擦声和短促噪声只能进入候选，不应直接停角色 TTS。
- Android 播放中打断门限收紧到约 `640ms / 6 帧`，额外要求至少 `4` 个连续稳定人声帧；WebRTC VAD 可用时要求 speech frame 占比不低于 `75%`，并叠加 RMS、过零率和 crest factor 声学门控。普通角色说完后的聆听阈值不变，避免角色讲完后用户轻声接话被漏掉。
- iOS 播放中打断门限收紧到约 `720ms / 8 帧`，额外要求至少 `5` 个连续能量稳定帧；peak / average 门限提高，并把 peak-average spread 收紧到 `18dB`，优先阻断持续轻噪音和外放回声误触发。
- 验收时先测“轻微桌面摩擦、轻拍桌面、空调/风扇、外放角色声音回采”不能频繁中断，再测正常近场说“等一下”仍能打断。

2026-06-29 角色卡住不说话/追问恢复热修记录：

Status: current。

- 静默追问顺序仍是 `followup_5s -> followup_30s -> goodbye_60s`，从角色上一段语音播放完成或失败恢复后开始计时；用户开始说话、提交 turn、打断或挂断会重置当前静默链路。
- 线上反馈“角色卡着半天不说话”后，客户端把 voice-call 流式 turn / idle-turn 超时从 `120s` 收紧到 `45s`。服务端或 TTS 长时间无响应时，客户端会快速退出等待，回到监听/追问恢复链路。
- Android / iOS 的 `onAssistantPlaybackFailed` 现在会立即重新启动静默追问计时，不再只清状态和等待下一次录音窗口超时；避免播放失败、空流、网络中断后用户体感角色沉默。
