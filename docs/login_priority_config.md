...
- 服务端当前对华为 `1.0.6`、`1.0.7`、`1.0.8` 内置提审态：`android_feature_flags.phone_login_entry_enabled=false`。iOS `1.0.7` 提审态关闭 `ios_feature_flags.phone_login_entry_enabled`，iOS `1.0.8` 提审态关闭 `ios_feature_flags.wechat_login_entry_enabled`，并保持 `ios_feature_flags.phone_login_entry_enabled=true`。
