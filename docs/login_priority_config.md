...
- 服务端当前对华为 `1.0.6`、`1.0.7` 内置提审态：`android_feature_flags.phone_login_entry_enabled=false`。iOS `1.0.7` 提审态同样关闭 `ios_feature_flags.phone_login_entry_enabled`。
...
5. Android 普通包打开登录页，确认微信为主按钮；远程 `phone_login_entry_enabled=true` 时点击底部“更多登录方式”后出现手机号图标。华为 `1.0.7` 提审包确认不展示手机号图标。
