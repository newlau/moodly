...
- 糖果的首充赠送不在 App Store Connect 配，而由服务端动态计算。当前新版首充策略：¥6 不送，¥18 送 50%，¥38 送 100%，¥128 送 120%；会员不再叠加充值额外 20% 加赠。
...
- 会员不再叠加充值额外 20% 加赠，`bonus_candy` / `total_candy` 仅表达首充赠送后的实际到账
