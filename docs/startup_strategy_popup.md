...
- 登录审核入口同样复用 `ios_feature_flags`：iOS `1.0.8` / `1.0.9` 提审态下发 `wechat_login_entry_enabled=false`，并保持 `phone_login_entry_enabled=true`；iOS `1.1.0` 正式态下发 `wechat_login_entry_enabled=true`，登录页恢复微信入口。
